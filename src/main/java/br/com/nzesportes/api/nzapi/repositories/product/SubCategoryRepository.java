...
public interface SubCategoryRepository extends JpaRepository<SubCategory, UUID> {
    ...
    @Query(value = "SELECT sc FROM SubCategory sc WHERE sc.category = :category AND sc.status = true AND sc.onMenu = true AND (sc.gender = :gender OR sc.gender = 'BOTH') ORDER BY sc.name ASC")
